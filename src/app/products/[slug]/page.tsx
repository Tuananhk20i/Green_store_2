...
export default function ProductDetailPage() {
  ...
  // 2. State để lưu trữ chuỗi base64 của ảnh QR
  ...
  const fetchProduct = async () => {
    try {
      ...
      if (data.success) {
        ...
        // 4. Gọi hàm tạo mã QR ngay khi có dữ liệu sản phẩm
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-lg shadow-sm border p-6">
          ...
          {/* 5. Hiển thị mã QR Truy xuất nguồn gốc */}
          <div className="mb-6 p-4 border rounded-lg bg-gray-50 flex items-center gap-4">
            {qrCodeUrl && (
              <div className="flex-shrink-0">
                <img src={qrCodeUrl} alt="QR Code" className="w-24 h-24 border bg-white p-1" />
              </div>
            )}
            <div>
              <h4 className="text-sm font-bold text-[#527a2d] uppercase">Truy xuất nguồn gốc</h4>
              <p className="text-xs text-gray-500 mt-1">
                Quét mã QR để xem chứng nhận và quy trình sản xuất sạch của <strong>{product.name}</strong>.
              </p>
            </div>
          </div>
